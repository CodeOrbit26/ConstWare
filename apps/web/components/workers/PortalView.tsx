...
export function PortalView() {
  ...
  React.useEffect(() => {
    if (workers && workers.length > 0) {
      ...
      workers.forEach((w: any) => {
        initialAccess[w.id] = w.status === 'available'
      })
  ...
  const filteredWorkers = (workers || []).filter(w => {
    ...
    const matchesSearch = getWorkerName(w).toLowerCase().includes(s) || (w.aadhaarNumber && w.aadhaarNumber.includes(s))
  ...
  const targetWorker = idCardWorker ? workers.find(w => w.id === idCardWorker) : null
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row gap-4 mt-4 w-full">
         ...
         <Select value={siteFilter} onValueChange={(val) => setSiteFilter(val || "all")}>
                ...
                <div key={worker.id} className="bg-[#111827] rounded-2xl p-4 border border-white/5 flex flex-col gap-3">
                   ...
                   <div className="flex items-center justify-between gap-2">
                      <div className="flex flex-col truncate">
                         <span className="font-extrabold text-sm text-white truncate">{getWorkerName(worker)}</span>
                    ...
                    <tr key={worker.id} className="hover:bg-slate-800/30 transition-colors">
                      <td className="p-5">
                         <div className="flex flex-col">
                           <span className="font-bold text-slate-200">{getWorkerName(worker)}</span>
             ...
             <div className="bg-white rounded-[2rem] w-[300px] h-[480px] overflow-hidden shadow-2xl relative flex flex-col items-center pt-8 border-4 border-slate-200">
                ...
                <div className="relative h-24 w-24 rounded-full border-4 border-white bg-slate-200 flex items-center justify-center text-slate-400 overflow-hidden shadow-md">
                   {targetWorker.photoUrl ? (
                      ...
                      <span className="text-3xl font-black">{getWorkerName(targetWorker).charAt(0)}</span>
                   )}
                </div>

                <h2 className="mt-4 text-2xl font-black text-slate-900 tracking-tight text-center px-4">{getWorkerName(targetWorker)}</h2>
