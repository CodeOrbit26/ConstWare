...
export function AttendanceView() {
  ...
  const saveAttendance = useFinanceStore((state: any) => state.saveAttendance)
  const getAttendance = useFinanceStore((state: any) => state.getAttendance)
  ...
  // Map worker profiles for dynamic PagarBook creation metadata
  const workerDetails = React.useMemo(() => {
    const details: Record<string, { name: string; dailyWage: number }> = {};
    workers.forEach((w: any) => {
      details[w.id] = {
        name: getWorkerName(w),
        dailyWage: w.dailyWage || w.daily_wage || 800
      };
    });
    return details;
  }, [workers]);
  ...
  const activeWorkers = React.useMemo(() => {
    return (workers || []).filter((w: any) => 
      ...
      (getWorkerName(w).toLowerCase().includes(search.toLowerCase()) || w.skill.toLowerCase().includes(search.toLowerCase()))
    )
  ...
  React.useEffect(() => {
    if (Object.keys(attendance).length > 0 || hasSubmitted) {
      saveAttendance(siteId, date, attendance, hasSubmitted, workerDetails)
    }
  }, [attendance, hasSubmitted, siteId, date, saveAttendance, workerDetails])
         ...
         <div className="p-3 md:p-4 flex flex-col gap-2.5">
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
               ...
               <div className="space-y-1">
                  ...
                  <Select value={siteId} onValueChange={(val) => setSiteId(val || "all")}>
                   ...
                   >
                      ...
                      <div className="flex-[2] flex items-center gap-3 min-w-0 pl-1 md:pl-0">
                         <div className={cn(
                            ...
                            {getWorkerName(worker).substring(0, 2).toUpperCase()}
                         </div>
