...
export default function WorkerProfileView() {
  ...
  const workerName = worker.name || `${worker.firstName || ""} ${worker.lastName || ""}`.trim() || "Unknown Worker"
  const initials = workerName.split(' ').map((n: string) => n[0]).join('')

  return (
    <DashboardLayout title={`${workerName} - Profile`}>
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-end gap-6">
            ...
            <div className="space-y-2 text-center sm:text-left">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <h1 className="text-3xl font-black text-navy dark:text-white tracking-tight">{workerName}</h1>
