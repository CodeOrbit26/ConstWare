...
export function Sidebar({ className, isCollapsed = false, onToggle }: SidebarProps) {
  ...
  const user = isSupervisor 
    ? { name: "Rahul Singh", role: "Site Supervisor", site: "Green Valley", initials: "RS" }
    : { 
  ...
  return (
    <div className={cn("flex h-full flex-col bg-slate-950 text-white border-r border-slate-800/50 backdrop-blur-xl relative transition-all duration-300 z-40 overflow-hidden", className)}>
      ...
      {/* Footer Profile Area */}
      <div className="mt-auto border-t border-slate-800 bg-slate-900/50 p-4 shrink-0 transition-all duration-300 overflow-hidden w-full">
        <div className="flex items-center mb-4 w-full">
          <div className="shrink-0 transition-all duration-300 flex items-center justify-center w-[52px]" title={isCollapsed ? user.name : undefined}>
            <Avatar initials={user.initials} size="sm" className="bg-slate-700 text-white shadow-sm" />
          </div>
          <div className={cn("flex flex-col whitespace-nowrap overflow-hidden transition-all duration-300 ease-in-out", isCollapsed ? "opacity-0 max-w-0 ml-0" : "opacity-100 max-w-[200px] ml-2")}>
        ...
        <div className="flex flex-col gap-2">
          {/* Settings Button */}
          <Link href="/contractor/settings" className="w-full">
            <Button 
              variant="ghost" 
              title={isCollapsed ? "Settings" : undefined}
              className="text-slate-400 hover:text-primary hover:bg-primary/10 transition-all duration-300 overflow-hidden w-full px-3 justify-start"
            >
              <div className="flex h-6 w-6 shrink-0 items-center justify-center">
          ...
          <Button 
            ...
            className="text-slate-400 hover:text-rose-500 hover:bg-rose-500/10 transition-all duration-300 overflow-hidden w-full px-3 justify-start"
            onClick={() => {}}
          >
            <div className="flex h-6 w-6 shrink-0 items-center justify-center">
