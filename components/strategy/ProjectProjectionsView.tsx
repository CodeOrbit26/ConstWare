...
export function ProjectProjectionsView() {
  ...
  const [activeTab, setActiveTab] = React.useState<'ESTIMATE' | '3D_RENDER' | 'INTERIOR' | 'BLUEPRINT_UPLOAD'>('ESTIMATE')
    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
         <div className="space-y-1">
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest border-l-2 border-primary pl-3">Precision Forecasting Engine v2.4</p>
         </div>
         <div className="flex items-center gap-2 bg-slate-900/50 border border-slate-800/50 p-1.5 rounded-2xl">
             {['ESTIMATE', '3D_VISUALIZER'].map((tab) => {
               ...
               return (
                 <button
                   key={tab}
                   onClick={() => handleTabChange(tab === '3D_VISUALIZER' ? '3D_VISUALIZER' : 'ESTIMATE')}
                   className={cn(
                 ...
                 >
                   ...
                   {tab === 'ESTIMATE' ? 'Estimate Setup' : '3D Visualizer'}
                 </button>
               )
            })}
         </div>
                 ...
                 alignItems: 'start'
                  ...
                  <FieldGroup label="Architectural Asset Sync" fullWidth={true}>
                    <p style={{ fontSize: '13px', color: '#4B5563', margin: '0 0 14px 0', lineHeight: '1.5' }}>
                      Upload floor blueprints for AI-powered precise estimation. One image per floor. Optional but improves accuracy.
                    </p>
