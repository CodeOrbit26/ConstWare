...
import { Users, CheckSquare, Wallet, KeyRound } from "lucide-react"
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6 w-full">
          ...
          <TabsList className="flex h-auto w-full p-0 bg-[#0A0F1E] rounded-xl border border-white/5 overflow-hidden shadow-md">
            ...
            <TabsTrigger 
              ...
              className={cn(
                "flex-1 p-3 sm:p-4 rounded-none border-b-[3px] border-transparent data-[state=active]:border-[#F97316] data-[state=active]:bg-[#111827] bg-transparent text-slate-500 data-[state=active]:text-white transition-all border-r border-white/5 last:border-r-0 focus-visible:ring-0",
                activeTab === 'payroll' ? "hidden md:flex flex-col items-center md:items-start gap-1" : "flex flex-col items-center md:items-start gap-1 justify-center"
              )}
            >
              <div className="flex items-center gap-1 sm:gap-2 font-black text-[11px] sm:text-sm uppercase tracking-wider text-slate-300">
                 <Wallet className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-500 shrink-0" /> 
                 <span className="truncate">Payouts</span>
              </div>
              <span className="text-xs font-medium text-slate-500 data-[state=active]:text-slate-400 normal-case hidden md:block truncate">Pay wages</span>
            </TabsTrigger>
          ...
          <TabsContent value="payroll" className="mt-0 focus-visible:ring-0 w-full">
             <PayrollView />
          </TabsContent>
